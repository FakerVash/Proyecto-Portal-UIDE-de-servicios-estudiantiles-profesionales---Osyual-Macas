...
export default async function servicioRoutes(fastify: FastifyInstance, opts: FastifyPluginOptions) {
    const controller = new ServicioController(fastify);

    // Ruta pública para ver servicios
    fastify.get('/', { schema: getServiciosSchema }, (req, rep) => controller.getAll(req as any, rep));

    // Ruta protegida para crear servicios (Solo estudiantes)
    fastify.post('/', {
        schema: createServicioSchema,
        onRequest: [fastify.authenticate, (fastify as any).checkRoles(['ESTUDIANTE', 'ADMIN'])]
